...
class TokenService {
    generateTokens(payload) {
        const accsessToken = jsonwebtoken.sign(payload, process.env.JWT_ACCESS_SECRET, { expiresIn: '15m' })
        const refreshToken = jsonwebtoken.sign(payload, process.env.JWT_REFRESH_SECRET, { expiresIn: '1h' })
        return {
            accsessToken,
            refreshToken,
        }
    ...
    async saveToken(user, refreshToken) {
        ...
        if (tokenData) {
            ...
            await user.createToken({ refreshToken })
        }
